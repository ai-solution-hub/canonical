...
// One branch per `type`; each payload is a Supabase RPC Json return (opaque).
const InsightsResponseSchema = z.union([
  z.object({ trends: z.array(z.unknown()) }),
  z.object({ topic: z.unknown() }),
  z.object({ author: z.unknown() }),
  z.object({ gaps: z.unknown() }),
  z.object({ reading: z.unknown() }),
]);
