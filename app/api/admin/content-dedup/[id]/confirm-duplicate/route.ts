 ...
 *
 * Admin resolves a suspected-duplicate row as an actual duplicate of an
 * existing canonical: archives the subject (sets `archived_at`,
 * `archived_by`, `archive_reason`), flips `dedup_status` to
 * `'confirmed_duplicate'`, and writes a `content_history` audit row.
 *
 * Idempotency: rejects with 409 when the subject is no longer in
 * `suspected_duplicate` (concurrent admin already resolved it).
 *
