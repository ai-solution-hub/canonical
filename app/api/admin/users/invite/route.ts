...
// 201 success body. `email` is the GoTrue User.email (string | undefined) —
// JSON.stringify drops it when undefined, so it is optional. `warnings` is set
// to undefined (key dropped) on the happy path, present only when role
// assignment failed.
const UserInviteResponseSchema = z.object({
  id: z.string(),
