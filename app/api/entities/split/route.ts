...
const EntitySplitResponseSchema = z.object({
  split: z.literal(true),
  original: z.string(),
  new_canonical_name: z.string(),
  mentions_moved: z.number(),
  all_mentions_moved: z.boolean(),
  relationships_updated: z.number(),
});

export const POST = defineRoute(
  EntitySplitResponseSchema,
        ...
        return NextResponse.json(
          {
            error: safeErrorMessage(
              updateErr,
              'Failed to split entity mentions',
            ),
          },
          { status: 500 },
        );
