  ...
  async (
    ...
    try {
      ...
      const { error: mapCountError } = await supabase
        ...
        .eq('id', id);
      if (mapCountError) {
        // Secondary counter write — the field mapping itself has landed, so
        // do not fail the request; make the drift observable instead.
        logBestEffortWarn(
          'procurement.form.mapcount',
          'Failed to update form_instances.mapped_count after field mapping',
          {
            formInstanceId: id,
            fieldId,
