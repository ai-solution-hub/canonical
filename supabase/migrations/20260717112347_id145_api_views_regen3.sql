...
--
-- Views:     63 security_invoker 1:1 views (explicit cols, FK verbatim).
...
GRANT SELECT ON api.engagement_group_content TO anon;
...
GRANT SELECT ON api.engagement_groups TO anon;
...
-- form_attachments ──────────────────────────────────────────────────────
DROP VIEW IF EXISTS api.form_attachments;
CREATE VIEW api.form_attachments WITH (security_invoker = true) AS
  SELECT
    id,
    form_instance_id,
    engagement_group_id,
...
GRANT SELECT ON api.form_attachments TO anon;
GRANT SELECT, INSERT, UPDATE, DELETE ON api.form_attachments TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON api.form_attachments TO service_role;
...
GRANT SELECT ON api.promotion_dispositions TO anon;
GRANT SELECT, INSERT, UPDATE, DELETE ON api.promotion_dispositions TO authenticated;
...
-- api.hybrid_search(query_embedding vector, query_text text, similarity_threshold numeric, limit_count integer, include_superseded boolean, visibility_filter character varying, application_type text, filter_kind text, filter_domain text, filter_subtopic text, filter_date_from timestamp with time zone, filter_date_to timestamp with time zone)  [INVOKER entrypoint]
