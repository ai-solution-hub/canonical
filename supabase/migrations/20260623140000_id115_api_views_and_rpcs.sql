...
CREATE FUNCTION api.get_dashboard_attention_counts(p_user_id uuid, p_role text DEFAULT 'viewer'::text)
  RETURNS TABLE(governance_review_count integer, unverified_count integer, quality_flag_count integer, stale_content_count integer, expired_content_count integer, expiring_content_date_count integer, unread_notification_count integer, coverage_gap_count integer, freshness_summary jsonb)
  LANGUAGE sql
  SECURITY INVOKER
  SET search_path = public, extensions
AS $api$
