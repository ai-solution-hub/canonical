...
-- Functions: 45 INVOKER entrypoints/wrappers (search_path=public,extensions).
...
-- record_embeddings ─────────────────────────────────────────────────────
DROP VIEW IF EXISTS api.record_embeddings;
CREATE VIEW api.record_embeddings WITH (security_invoker = true) AS
  SELECT
    id,
    owner_kind,
    owner_id,
...
GRANT SELECT ON api.record_embeddings TO anon;
GRANT SELECT, INSERT, UPDATE, DELETE ON api.record_embeddings TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON api.record_embeddings TO service_role;
...
CREATE VIEW api.record_lifecycle WITH (security_invoker = true) AS
  SELECT
    ...
    source_document_id,
    q_a_pair_id,
    owner_id,  -- generated: passthrough (selectable, never insertable)
    domain,
...
GRANT SELECT ON api.record_lifecycle TO anon;
GRANT SELECT, INSERT, UPDATE, DELETE ON api.record_lifecycle TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON api.record_lifecycle TO service_role;
...
-- api.citations_cascade_preflight()  [INVOKER wrapper over SECURITY DEFINER public fn]
...
-- api.corpus_writer_fence_lease_acquire(p_holder_token uuid, p_holder text, p_ttl_seconds integer)  [INVOKER wrapper over SECURITY DEFINER public fn]
DROP FUNCTION IF EXISTS api.corpus_writer_fence_lease_acquire(p_holder_token uuid, p_holder text, p_ttl_seconds integer);
...
$api$;
REVOKE EXECUTE ON FUNCTION api.corpus_writer_fence_lease_acquire(p_holder_token uuid, p_holder text, p_ttl_seconds integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION api.corpus_writer_fence_lease_acquire(p_holder_token uuid, p_holder text, p_ttl_seconds integer) TO authenticated, service_role;

-- api.corpus_writer_fence_lease_release(p_holder_token uuid, p_holder text)  [INVOKER wrapper over SECURITY DEFINER public fn]
DROP FUNCTION IF EXISTS api.corpus_writer_fence_lease_release(p_holder_token uuid, p_holder text);
...
$api$;
REVOKE EXECUTE ON FUNCTION api.corpus_writer_fence_lease_release(p_holder_token uuid, p_holder text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION api.corpus_writer_fence_lease_release(p_holder_token uuid, p_holder text) TO authenticated, service_role;

-- api.corpus_writer_fence_release(p_holder text)  [INVOKER wrapper over SECURITY DEFINER public fn]
...
-- api.corpus_writer_fence_try_acquire(p_holder text)  [INVOKER wrapper over SECURITY DEFINER public fn]
...
-- api.get_content_win_rate(p_q_a_pair_id uuid)  [INVOKER entrypoint]
DROP FUNCTION IF EXISTS api.get_content_win_rate(p_q_a_pair_id uuid);
CREATE FUNCTION api.get_content_win_rate(p_q_a_pair_id uuid)
  ...
  SET search_path = public, extensions
AS $api$
  SELECT * FROM public.get_content_win_rate(p_q_a_pair_id => p_q_a_pair_id);
$api$;
REVOKE EXECUTE ON FUNCTION api.get_content_win_rate(p_q_a_pair_id uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION api.get_content_win_rate(p_q_a_pair_id uuid) TO authenticated, service_role;
...
CREATE FUNCTION api.get_dashboard_attention_counts(p_user_id uuid, p_role text DEFAULT 'viewer'::text)
  RETURNS TABLE(governance_review_count integer, unverified_count integer, quality_flag_count integer, stale_content_count integer, expired_content_count integer, expiring_content_date_count integer, unread_notification_count integer, freshness_summary jsonb)
  LANGUAGE sql
  SECURITY INVOKER
  SET search_path = public, extensions
AS $api$
...
-- api.hybrid_search(query_embedding vector, query_text text, similarity_threshold numeric, limit_count integer, include_superseded boolean, visibility_filter character varying, application_type text)  [INVOKER entrypoint]
...
CREATE FUNCTION api.hybrid_search(query_embedding vector, query_text text DEFAULT ''::text, similarity_threshold numeric DEFAULT 0.3, limit_count integer DEFAULT 10, include_superseded boolean DEFAULT false, visibility_filter character varying DEFAULT 'default'::character varying, application_type text DEFAULT 'procurement'::text)
  ...
  SET search_path = public, extensions
AS $api$
  SELECT * FROM public.hybrid_search(query_embedding => query_embedding, query_text => query_text, similarity_threshold => similarity_threshold, limit_count => limit_count, include_superseded => include_superseded, visibility_filter => visibility_filter, application_type => application_type);
$api$;
REVOKE EXECUTE ON FUNCTION api.hybrid_search(query_embedding vector, query_text text, similarity_threshold numeric, limit_count integer, include_superseded boolean, visibility_filter character varying, application_type text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION api.hybrid_search(query_embedding vector, query_text text, similarity_threshold numeric, limit_count integer, include_superseded boolean, visibility_filter character varying, application_type text) TO anon, authenticated, service_role;
...
CREATE FUNCTION api.merge_entities(p_source_names text[], p_target_name text, p_entity_type text)
  RETURNS TABLE(merged boolean, target text, entity_type text, mentions_updated integer, relationship_sources_updated integer, relationship_targets_updated integer, duplicates_removed integer)
  LANGUAGE sql
  SECURITY INVOKER
  SET search_path = public, extensions
AS $api$
...
-- api.reap_orphaned_source_documents()  [INVOKER wrapper over SECURITY DEFINER public fn]
...
-- api.resolve_or_mint_source_identity(p_content_hash text, p_rel_path text, p_filename text, p_mime_type text, p_file_size integer, p_origin_type text, p_retention_class text, p_op_id uuid)  [INVOKER wrapper over SECURITY DEFINER public fn]
...
CREATE FUNCTION api.search_for_form_response(query_embedding vector, query_text text DEFAULT ''::text, limit_count integer DEFAULT 10, include_superseded boolean DEFAULT false, visibility_filter character varying DEFAULT 'default'::character varying)
  RETURNS TABLE(id uuid, title text, content text, content_type text, summary text, similarity numeric)
  LANGUAGE sql
  SECURITY INVOKER
  SET search_path = public, extensions
AS $api$
...
-- api.tombstone_source_document(p_id uuid)  [INVOKER wrapper over SECURITY DEFINER public fn]
