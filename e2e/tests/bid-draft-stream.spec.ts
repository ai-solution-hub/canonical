...
/**
 * Fingerprint for text that is STORED MARKDOWN (a `response_text` column, or a
 * seed constant destined for one), as opposed to text read out of the rendered
 * editor.
 *
 * `normaliseText` claims to eliminate list bullets, and it does — for `-`/`*`,
 * which are punctuation. It does NOT eliminate ORDERED-list markers, because
 * those are digits and digits are content. The editor renders `1. ` as an
 * `<ol>` marker generated by CSS, which is absent from `textContent`, so the
 * marker exists on the DB side of every DB-vs-editor comparison and on neither
 * side of the rendered one:
 *
 *   DB     "3. 3rd Line Support"  -> "33rdlinesupport"
 *   editor "3rd Line Support"     -> "3rdlinesupport"
 *
 * That is a false mismatch, and a nasty one: it fires only when the model
 * happens to emit a numbered list, so it presents as intermittent. It was
 * invisible until now because these two specs failed earlier, at the Anthropic
 * call, on every run since they were written.
 *
 * Stripping the marker does not weaken the comparison. Everything else — every
 * letter and every digit of the actual content, in order — must still match
 * exactly, so a truncated, empty or divergent editor still fails.
 */
function normaliseStoredMarkdown(input: string): string {
  return normaliseText(input.replace(/^[ \t]*\d+[.)][ \t]+/gm, ' '));
}
...
test.describe('Procurement draft-stream happy path (8.0.7)', () => {
  ...
  test('Regenerate streams a new draft, persists to DB, renders in editor', async ({
    ...
    const expectedNormalised = normaliseStoredMarkdown(row.response_text ?? '');
    let lastActual = '';
    await expect
      .poll(
        async () => {
...
test.describe('Procurement regenerate + restore (8.0.8)', () => {
  ...
  test('Regenerate creates a v2 snapshot, then restore brings v1 forward as v3', async ({
    ...
    await expect
      .poll(
        async () => {
          ...
          if (text === normaliseStoredMarkdown(originalText)) return 'original';
          return 'regenerated';
        },
        {
          timeout: 30000,
    ...
    expect(normaliseText(regeneratedText)).not.toBe(
      normaliseStoredMarkdown(originalText),
    );
    ...
    const postRegenDbNorm = normaliseStoredMarkdown(
      postRegenRows.response_text ?? '',
    );
    ...
    expect(normaliseStoredMarkdown(v2Row!.response_text ?? '')).toBe(
