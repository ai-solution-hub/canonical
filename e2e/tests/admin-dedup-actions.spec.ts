...
async function resolveAdminActorId(supabase: SupabaseClient): Promise<string> {
  ...
  if (!adminEmail) {
    throw new Error(
      'admin-dedup-actions: TEST_USER_1_EMAIL not set. ' +
  ...
  if (error) {
    throw new Error(
      `admin-dedup-actions: auth.admin.listUsers failed — ${error.message}`,
    );
  }
  ...
  if (!adminUser) {
    throw new Error(
      `admin-dedup-actions: admin user with email ${adminEmail} not found. ` +
