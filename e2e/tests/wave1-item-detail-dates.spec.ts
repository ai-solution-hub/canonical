...
test.describe('Item detail — expiry date display', { tag: '@wave1' }, () => {
  test('expired item shows expiry date in metadata sidebar', async ({ authenticatedPage: page, workerData }) => {
  ...
  test('expiry date uses DD/MM/YYYY UK format', async ({ authenticatedPage: page, workerData }) => {
    await page.goto(`/item/${workerData.expiredItemId}`);
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible({ timeout: 10000 });
  ...
  test('expired item shows urgency status badge', async ({ authenticatedPage: page, workerData }) => {
    await page.goto(`/item/${workerData.expiredItemId}`);
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible({ timeout: 10000 });

    // ExpiryDateDisplay renders a role="status" element with aria-label="Expiry status: {label}"
    const statusBadge = page.locator('[role="status"][aria-label^="Expiry status:"]');
  ...
  test('date-bound item shows lifecycle type indicator', async ({ authenticatedPage: page, workerData }) => {
    await page.goto(`/item/${workerData.expiredItemId}`);
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible({ timeout: 10000 });
  ...
  test('item without expiry date does not show expiry section', async ({ authenticatedPage: page, workerData }) => {
...
test.describe('Item detail — temporal references section', { tag: '@wave1' }, () => {
  test('temporal references section is collapsible', async ({ authenticatedPage: page, workerData }) => {
  ...
  test('expanded temporal references show date details', async ({ authenticatedPage: page, workerData }) => {
    await page.goto(`/item/${workerData.expiredItemId}`);
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible({ timeout: 10000 });
    ...
    if (await toggleButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (itemCount > 0) {
        ...
        const contextTypes = ['Expiry', 'Effective', 'Review', 'Publication', 'Historical', 'Unknown'];
        ...
        const confidenceSpan = firstItem.locator('span[aria-label^="Confidence:"]');
        if (await confidenceSpan.isVisible({ timeout: 2000 }).catch(() => false)) {
          const confidenceLabel = await confidenceSpan.getAttribute('aria-label');
          expect(confidenceLevels.some((level) => confidenceLabel?.includes(level))).toBe(true);
        }
      }
    }
  });

  test('temporal references section shows count in button text', async ({ authenticatedPage: page, workerData }) => {
    await page.goto(`/item/${workerData.expiredItemId}`);
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible({ timeout: 10000 });
  ...
  test('temporal references list has accessible label', async ({ authenticatedPage: page, workerData }) => {
    await page.goto(`/item/${workerData.expiredItemId}`);
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible({ timeout: 10000 });
...
test.describe('Item detail — quality score badge', { tag: '@wave1' }, () => {
  test('browse page content cards show quality score badges', async ({ authenticatedPage: page }) => {
    ...
    if (badgeCount > 0) {
      ...
      const scoreText = await firstBadge.locator('span.font-semibold').textContent();
  ...
  test('quality badge uses semantic colour tokens', async ({ authenticatedPage: page }) => {
    await page.goto('/browse');
    await expect(page.getByText(/^\d+ items?$/).first()).toBeVisible({ timeout: 10000 });
  ...
  test('quality badge shows breakdown in title attribute', async ({ authenticatedPage: page }) => {
    await page.goto('/browse');
    await expect(page.getByText(/^\d+ items?$/).first()).toBeVisible({ timeout: 10000 });
