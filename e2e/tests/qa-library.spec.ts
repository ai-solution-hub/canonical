...
async function gotoLibraryFiltered(page: Page, prefix: string) {
  ...
  await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 20000 });
  ...
  await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 15000 });
}
...
test.describe('Q&A Library page', () => {
  test.beforeAll(async ({ workerData }) => {
    ...
    extraQAIds = await Promise.all([
      ...
      createTestQAPair(prefix, 'People & Skills', {
        ...
        answer_standard: null,
        answer_advanced: 'Advanced retention framework with mentorship pipelines.',
      }),
    ]);
  });
  ...
  test('library page loads with heading', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    await expect(
  ...
  test('shows Q&A pair count summary', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    await expect(
      page.getByText(/\d+ Q&A pairs?/),
    ).toBeVisible({ timeout: 10000 });
  });

  test('shows standard and advanced counts', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    ...
    const hasStandardOrAdvanced = /\d+ standard/.test(summaryText ?? '') || /\d+ advanced/.test(summaryText ?? '');
  ...
  test('Q&A row shows item title', async ({ authenticatedPage: page, workerData }) => {
  ...
  test('Q&A row expands on click to show answer', async ({ authenticatedPage: page, workerData }) => {
    await gotoLibraryFiltered(page, workerData.prefix);
  ...
  test('expanded Q&A row shows copy button', async ({ authenticatedPage: page, workerData }) => {
    await gotoLibraryFiltered(page, workerData.prefix);
    ...
    await expect(
      slaRow.getByRole('button', { name: /copy/i }),
    ).toBeVisible();
  });
  ...
  test('search input is visible with placeholder', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    const searchInput = page.getByLabel('Search Q&A pairs');
    await expect(searchInput).toBeVisible();
    await expect(searchInput).toHaveAttribute('placeholder', 'Search questions and answers...');
  });

  test('typing in search filters the list', async ({ authenticatedPage: page, workerData }) => {
    await page.goto('/library');
    await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 20000 });
  ...
  test('clearing search restores full list', async ({ authenticatedPage: page, workerData }) => {
    await page.goto('/library');
    await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 20000 });
  ...
  test('domain filter dropdown is visible', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 20000 });
    // The domain filter trigger shows "All domains" — use button[role=combobox] with text
    const domainTrigger = page.locator('button[role="combobox"]').filter({ hasText: 'All domains' });
    await expect(domainTrigger).toBeVisible();
  });

  test('selecting a domain filters the list', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 20000 });
  ...
  test('clearing domain filter restores all items', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 20000 });
    ...
    const filteredTrigger = page.locator('button[role="combobox"]').filter({ hasText: selectedDomain ?? '' });
    await filteredTrigger.click();
    await page.locator('[role="option"]').filter({ hasText: 'All domains' }).click();
  ...
  test('freshness filter dropdown is visible', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 20000 });
    const freshnessTrigger = page.locator('button[role="combobox"]').filter({ hasText: 'All freshness' });
  ...
  test('more filters button opens popover', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 20000 });
  ...
  test('variant filter shows options', async ({ authenticatedPage: page }) => {
    ...
    const variantTrigger = popover.locator('button[role="combobox"]').filter({ hasText: 'All variants' });
    await variantTrigger.click();

    // Verify all variant options exist
    await expect(page.locator('[role="option"]').filter({ hasText: 'Standard + Advanced' })).toBeVisible();
    await expect(page.locator('[role="option"]').filter({ hasText: 'Standard only' })).toBeVisible();
    await expect(page.locator('[role="option"]').filter({ hasText: 'Advanced only' })).toBeVisible();
    await expect(page.locator('[role="option"]').filter({ hasText: 'No answer' })).toBeVisible();
  });

  test('grouping by domain creates collapsible groups', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 20000 });
    ...
    const popover = page.locator('[data-radix-popper-content-wrapper]');
    const groupingTrigger = popover.locator('button[role="combobox"]').filter({ hasText: 'No grouping' });
    await groupingTrigger.click();
    await page.locator('[role="option"]').filter({ hasText: 'By domain' }).click();
  ...
  test('searching for nonsense shows empty state', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 20000 });
  ...
  test('select-all checkbox is visible', async ({ authenticatedPage: page }) => {
    await page.goto('/library');
    await expect(page.getByText(/\d+ Q&A pairs?/)).toBeVisible({ timeout: 20000 });
    await expect(
      page.getByLabel(/select all/i),
    ).toBeVisible();
  });

  test('selecting items shows bulk action toolbar', async ({ authenticatedPage: page, workerData }) => {
    // Filter to worker items so they appear in the virtualised list
    await gotoLibraryFiltered(page, workerData.prefix);
  ...
  test('select all then deselect all', async ({ authenticatedPage: page, workerData }) => {
  ...
  test('Q&A Library is accessible via header navigation', async ({ authenticatedPage: page }) => {
  ...
  test('clicking detail link navigates to item detail', async ({ authenticatedPage: page, workerData }) => {
  ...
  test('filters stack vertically on mobile', async ({ authenticatedPage: page }) => {
  ...
  test('Q&A rows are tappable on mobile', async ({ authenticatedPage: page, workerData }) => {
    if (!isMobileViewport(page)) {
      test.skip();
      return;
    }
  ...
  test('viewer does not see bulk action checkboxes', async ({ viewerPage: page, workerData }) => {
    ...
    const selectAll = page.getByLabel(/select all/i);
    const isSelectAllVisible = await selectAll.isVisible({ timeout: 5000 }).catch(() => false);
