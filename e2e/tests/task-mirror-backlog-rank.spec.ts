...
function buildBacklogFixture(): BacklogFixture {
  return {
    document_name: 'Product Backlog',
    document_purpose:
      'Subtask 30.8 fixture — three items per tier for rank/drag tests.',
...
baseTest.describe(
  'Subtask 30.8 — Backlog rank + drag-reorder (task-view sibling)',
  () => {
    ...
    baseTest.describe('Markup contracts (SSR — operable today)', () => {
      ...
      baseTest(
        ...
        async ({ page }) => {
          ...
          await expect(
            page.locator('table[data-backlog-table]'),
          ).toHaveAttribute('data-supports-drag-reorder', 'true');
        },
      );
      ...
      baseTest(
        ...
        async ({ page }) => {
          ...
          await expect(rank4Cell.locator('.record-view-rank-value')).toHaveText(
            '—',
          );
        },
      );
      ...
      baseTest(
        ...
        async ({ page }) => {
          ...
          await expect(page.locator('[data-promote-affordance]')).toHaveCount(
            0,
          );
        },
      );
    ...
    baseTest.describe(
      'Patch-server end-to-end (atomic multi-field rank PATCH)',
      () => {
        ...
        baseTest(
          ...
          async () => {
            ...
            const patchResp = await apiContext.patch('/api/ledger/record/1', {
              data: {
                baseMtime,
                patches: [
    ...
    baseTest.describe(
      'Interactive drag-drop + keyboard reorder (PENDING SPA hydration)',
      () => {
