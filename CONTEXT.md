...
- **activity** — the unit that carries structured data and its own id: a form, a proposal,
  a guide. Records scope to the activity, never to a workspace.
...
- **form** — a submission/questionnaire instance (`form_instances` row of a `form_type`) —
...
tenant (one DB per client) ⊃ applications (application_type) ⊃ activities (form / proposal
/ guide, each with its own id) ⊃ their child records; q_a_pairs are client-level, not
activity-partitioned. The workspace tier is retired from this chain.
...
concept files), **anti-tags**, **workspace as a scoping tier**.
