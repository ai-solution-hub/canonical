...
- **Review vs governance review — UNSETTLED, ruling pending (S521).** Owner position: they are ONE concern (curation + governance of the ingested + extracted corpus), the two surfaces being an artefact of the original IMS fork. **This is not yet supported by the register and needs a decision record amending DR-041**, which is in force and separates them at the top level ("curation surfaces live in Governance", vs Knowledge). Note DR-034 does NOT support the merge — it retires the content_items-era *coverage* feature while explicitly retaining **governance** as a critical signal. The closest live support is id-71 {71.9}'s single faceted queue (`facet: content_quality|governance|all`), and that subtask records the two route layers as **unchanged**. Until the amending DR is written, treat the surfaces as distinct in code and do not cite DR-034 for the merge.
