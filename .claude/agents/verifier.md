...
6) **“Compose existing backend only” is NOT binding (DR-062).** Never fail a task solely for adding backend (route, RPC, migration, helper) the specified behaviour requires — judge the behaviour, flag surprises as findings.
