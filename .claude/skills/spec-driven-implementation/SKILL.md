...
Where `<id>` is one of (matches `write-product-spec` / `write-tech-spec` conventions):

- a GitHub issue id, prefixed with `gh-` (e.g. `docs/specs/gh-4567/PRODUCT.md`)
- a Linear ticket number if Liam is using one for the feature (e.g. `docs/specs/APP-1234/PRODUCT.md`)
- a short kebab-case feature name (e.g. `docs/specs/q-a-workspace-scoping/PRODUCT.md`)
- once Taskmaster is installed (S232 WP4+), align `<id>` with the Taskmaster task ID for the feature so `docs/specs/<id>/` maps cleanly to the task tree.

`docs/specs/` should contain only id-named directories as direct children. Do not create engineer-named subdirectories or feature-slug-plus-suffix variants there.
...
Reference the GitHub issue or Linear ticket in the spec when one exists. Because specs live under `docs/specs/<id>/...`, this should usually be straightforward.
...
- the implementation spans multiple subsystems (e.g. Supabase + Python pipeline + Next.js + MCP)
...
- Vitest unit / integration tests under `__tests__/` for TS/Next.js code (`bun run test`, `bun run test:integration`)
...
- MCP eval suites (`bun run test:mcp-eval` / `:rq` / `:fc`) when the change touches MCP tools, resources, or prompts
- screenshots, recordings, or staging walkthroughs (https://knowledge-hub-git-staging-tw-group.vercel.app) when useful for UI-heavy work
...
- Keep tech specs implementation-oriented and grounded in current codebase patterns (`.planning/codebase/STRUCTURE.md`, `docs/reference/SCHEMA-QUICK-REFERENCE.md`, `docs/generated/codebase-stats.md`).
