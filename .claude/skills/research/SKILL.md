...
- **Decision register**: read the in-force table in `${KH_PRIVATE_DOCS_DIR}/src/content/docs/reference/decision-register.md` (a generated index), then open only the relevant files under `reference/decisions/`. Cite the governing DR wherever the task touches settled ground; never re-derive or re-litigate a settled ruling.
