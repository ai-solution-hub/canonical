...
Approved specs should live directly under an id-named directory in `docs/specs/`, for example `docs/specs/gh-4567/PRODUCT.md`, `docs/specs/APP-1234/PRODUCT.md`, or `docs/specs/q-a-workspace-scoping/PRODUCT.md` (and the matching `TECH.md` alongside). This matches the conventions used by `write-product-spec` / `write-tech-spec` / `spec-driven-implementation`.
...
Make sure you understand the expected behavior, constraints, risks, and validation plan before writing code. Also read the Knowledge Hub conventions the tech spec is grounded in:

- `.planning/codebase/STRUCTURE.md` — authoritative directory layout
- `docs/reference/SCHEMA-QUICK-REFERENCE.md` and `supabase/types/database.types.ts` — current schema (never hand-edit the generated types file)
...
Break the work into concrete implementation steps (use `planning-and-task-breakdown` if the work is large), then implement the feature against the approved specs.

During implementation:

- keep behavior aligned with `PRODUCT.md`
- keep architecture and sequencing aligned with `TECH.md`
- add or update tests and verification artifacts as the work lands (use `test-driven-development` and `incremental-implementation`)
- when DDL is needed, use the Supabase CLI (`supabase migration new` + `db push`), never MCP `execute_sql` for schema changes
- when MCP tools, resources, or prompts change, run `bun run generate:mcp-inventory` and update fixtures so guard tests pass
...
- Vitest unit / integration tests under `__tests__/` (`bun run test`, `bun run test:integration`)
...
- MCP eval suites (`bun run test:mcp-eval`, `:rq`, `:fc`) when the change touches MCP surfaces
- `bun lint`, `bun run format:check`, and `bun run knip` before merge for deterministic quality gates

See `docs/reference/test-philosophy.md` for the testing audit criteria and antipatterns to avoid.
...
- Respect parallel-worktree rules from `CLAUDE.md` when running multiple agents — sequential merges, `git reset --hard main` first, cherry-pick rather than merge for stale agent branches.
