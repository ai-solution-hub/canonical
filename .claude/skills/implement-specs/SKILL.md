...
description:
  Implement an approved feature from PRODUCT.md and TECH.md, keeping specs and
  code aligned in the same PR as implementation evolves. Use after the product
  and tech specs are approved and the next step is building the feature.
---
...
In many cases, the implementation should be pushed in the same PR as the product
and tech specs. As the engineer iterates, changes to `PRODUCT.md`, `TECH.md`,
and the code should all be pushed in that same PR so review stays anchored to
the feature that will actually ship.
...
- `TECH.md` as the source of truth for architecture, sequencing, and
...
For large or long-running features, optionally offer one of these aids to the
...
- `DECISIONS.md` to capture concrete product and technical decisions made during
  the PRD and tech design process

These are optional aids, not required deliverables. Offer them when they would
reduce confusion or help future agents avoid re-exploring the same paths.

### 3. Plan and implement against the specs

Break the work into concrete implementation steps (use
`planning-and-task-breakdown` if the work is large), then implement the feature
...
- when DDL is needed, use the Supabase CLI (`supabase migration new` +
  `db push`), never MCP `execute_sql` for schema changes
- when MCP tools, resources, or prompts change, run
  `bun run generate:mcp-inventory` and update fixtures so guard tests pass

Use the same PR for the specs and implementation when practical so the full
feature evolution is reviewable in one place.
...
- update `PRODUCT.md` when user-facing behavior, UX, edge cases, or success
  criteria change
- update `TECH.md` when architecture, sequencing, module boundaries, or
  validation strategy change
- keep those updates in the same PR as the corresponding code changes

The PR should describe the feature that actually ships, not just the initial
draft of the specs.
...
- Use optional tracking documents only when they add real value for a complex
  feature.
- Keep the same PR coherent: spec updates, code changes, tests, and optional
  tracking docs should all support the same feature narrative.
- Respect parallel-worktree rules from `CLAUDE.md` when running multiple agents
  — sequential merges, `git reset --hard main` first, cherry-pick rather than
  merge for stale agent branches.
