...
### Using Freshness in Form Responses

| Freshness | Form response guidance |
|-----------|----------------------|
...
| Expired | Warn explicitly: "This source is expired and should not be used in a form without verification" |
...
| **bid_discovered** | Content discovered during form preparation — may be temporary | Quarterly |

### Lifecycle Assignment Guidance

- **Policies and certifications**: `date_bound` (tied to renewal dates)
- **Q&A pairs from form library imports**: `bid_discovered` (verify and promote to `evergreen` after review)
...
1. **Items referenced in active forms** — these affect live submissions
...
| Priority | Meaning |
|----------|---------|
| **high** | Critical content — actively used in forms, core policies |
...
3. Whether referenced in active forms
...
- Content was created for a specific form that has concluded
- Content is a duplicate of a better-maintained item

## Interpreting Dashboard Health Data

When the `whats_in_my_queue` tool returns KB health data, interpret it as:
...
- Active forms have >75% question completion
...
- Active forms have 50-75% question completion

### Poor Health

- <60% of items are fresh
- >15% are stale or expired
- Multiple quality flags in form-critical domains
- Active forms have <50% question completion
...
| Moderate | Schedule a review session, prioritise stale items in form-active domains |
| Poor | Urgent: focus on expired items in domains used by active forms, then work outwards |

## Anti-Patterns

**Do not:**
- Treat all stale content as equally urgent — prioritise by form impact and priority level
- Archive content without checking if it is referenced in active forms
...
- Ignore freshness warnings in form responses — using expired content in a tender is a submission risk
...
**Do:**
- Always sort attention items by form impact first, then priority, then freshness severity
...
- Flag expired content used in active forms as critical, not just awareness
- Note when a freshness transition is routine (evergreen content aging) vs concerning (form-critical content going stale)
...
- **@completing-forms** — How freshness affects form response quality
