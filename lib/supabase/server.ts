 ...
 * `clientEnv`, which is Zod-validated at boot in `lib/env.ts` — guarantees
 * non-empty values, so the previous defensive `if (!supabaseUrl)` checks
 * are redundant here.
 */
 ...
 *
 * Reads URL from `clientEnv.NEXT_PUBLIC_SUPABASE_URL` and the secret key
 * from `serverEnv.SUPABASE_SECRET_KEY` — both validated at boot.
 */
