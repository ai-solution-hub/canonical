 ...
 * at S264 (the proof-of-concept doc at `docs/specs/workflow-evaluation/retro-S264.md`
 * is the first migrated record).
 *
 * Schema decisions:
 ...
 *     `SESSION_ID_REGEX` (`/^S\d+$/`) rather than `BARE_ID_REGEX`.
 ...
 *   - 4 soft-delete / adjudication fields per S271 §13.4 (cheap-now / expensive-to-
 *     retrofit): `deprecated` (default false), `deprecation_reason` (nullable),
 *     `superseding_record_id` (nullable), `last_conflict_check` (nullable ISO).
 *     These will power `{48.14}` evaluate-findings adjudication.
 *   - NO `version` / `$schema` root field — matches the three existing ledgers.
 *   - NO record-level `status` field — retros are records of past events, not
 *     in-flight work. (Per-record adjudication is captured in the soft-delete
 *     fields above, not a status enum.)
 *
 * Per RESEARCH §5 (workflow-evaluation programme) + S271 §13.4 schema delta.
 */
...
//
// Shape: `{ text, cross_doc_links? }`. `text` is the finding prose (one-liner or
// short paragraph). `cross_doc_links` is the same DocLinkSchema the other three
...
// §13.4 are baked in from the OUTSET (cheap-now / expensive-to-retrofit).
// ──────────────────────────────────────────────────────────────────────────────
...
export const RetroRecordSchema = z
  .object({
     ...
     * because retros are session-scoped.
     */
    id: z
      .string()
      .regex(
     ...
     * Kept distinct from `id` for forward compatibility with multi-track sessions
     * sharing a session number.
     */
    ...
    //
    // Per-record-first adjudication (OQ-S271-3 — simpler, matches the memory-
    // transcript precedent). Per-finding granularity may be added later if
    // needed. These four fields power {48.14} evaluate-findings later.

    /**
     * Soft-delete flag. False (default) = active record; true = superseded /
     * deprecated by adjudication. Defaults to false so existing records that
     * omit this field continue to parse as active.
     */
    deprecated: z.boolean().default(false),

    /**
     * Human-readable reason this record was deprecated (e.g.
     * "conflict-resolution:superseded-by:S2XX"). Null when not deprecated;
     * defaults to null when omitted from input.
     */
    deprecation_reason: z.string().nullable().default(null),

    /**
     * Pointer to the retro record that supersedes this one. Null when not
     * superseded; defaults to null when omitted. References another retro
     * `id` (session id form).
     */
     ...
     * adjudicated this record. Null = never adjudicated = eligible for next
     * sweep. Defaults to null when omitted.
     */
...
//
// At present no per-record char budgets are registered in the ledger-budgets
// registry for the retro surface (retros are append-only narrative — the
// six-category structure already enforces discipline). The helper exists for
// API parity with the three existing parse helpers; the returned warnings array
// is always empty until budgets are added in a future subtask.
// ──────────────────────────────────────────────────────────────────────────────
 ...
 * `warnings` array — empty for now; populated when per-record budgets are
 * registered in a future subtask.
 */
export function parseRetrosWithWarnings(input: unknown): {
  ...
  // No budgets registered for the retro surface yet — placeholder for future
  // {48.x} char-budget work mirroring parseRoadmapWithWarnings / parseBacklogWithWarnings.
