...
export function formatDocumentDiff(data: DocumentDiffData): string {
  ...
  // Summary
  lines.push('## Summary');
  lines.push(`- **Added:** ${data.summary.added} new Q&A ${data.summary.added === 1 ? 'pair' : 'pairs'}`);
  lines.push(`- **Removed:** ${data.summary.removed} Q&A ${data.summary.removed === 1 ? 'pair' : 'pairs'}`);
  lines.push(`- **Modified:** ${data.summary.modified} Q&A ${data.summary.modified === 1 ? 'pair' : 'pairs'} changed`);
  lines.push(`- **Unchanged:** ${data.summary.unchanged} Q&A ${data.summary.unchanged === 1 ? 'pair' : 'pairs'} identical`);
  lines.push('');
