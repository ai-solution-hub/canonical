...
export function formatCoverageMatrix(data: CoverageMatrixData): string {
  ...
  if (data.domains.length > 0) {
    ...
    for (const domain of data.domains) {
      lines.push(
        `| ${domain.name} | ${domain.total_items} | ${domain.fresh} | ${domain.aging} | ${domain.stale} | ${domain.expired} |`,
      );
    }
    lines.push('');
  }
  ...
  if (data.quality.total_flagged > 0) {
    ...
    const sortedTypes = Object.entries(data.quality.by_issue_type).sort(
      ([, a], [, b]) => b - a,
    );
