...
export async function registerQualityTools(server: McpServer): Promise<void> {
  ...
  server.registerTool(
    'get_quality_briefing',
    {
      ...
      description: 'Get an aggregated quality intelligence briefing covering below-threshold items, score drops, freshness transitions, quality flags, coverage alerts, and certification warnings. Supports optional domain filter and quality threshold override. Use this for proactive quality monitoring.',
      inputSchema: {
        domain: z.string().optional().describe('Filter to a specific domain (exact match on primary_domain)'),
        threshold: z.number().min(0).max(100).optional().describe('Override quality score threshold (default: from governance_config, fallback 40)'),
      },
      annotations: {
        readOnlyHint: true,
        idempotentHint: true,
    ...
    async (args, extra: ToolExtra) => {
      try {
        ...
        type QualityBriefingData = import('@/lib/mcp/formatters/briefing').QualityBriefingData;
        type BelowThresholdItem = import('@/lib/mcp/formatters/briefing').BelowThresholdItem;
        type ScoreDropItem = import('@/lib/mcp/formatters/briefing').ScoreDropItem;
        type FreshnessTransitionItem = import('@/lib/mcp/formatters/briefing').FreshnessTransitionItem;
        type QualityFlagNotification = import('@/lib/mcp/formatters/briefing').QualityFlagNotification;
        type CoverageAlertNotification = import('@/lib/mcp/formatters/briefing').CoverageAlertNotification;
        type CertificationWarning = import('@/lib/mcp/formatters/briefing').CertificationWarning;

        const domainFilter = args.domain || undefined;
        const thresholdOverride = args.threshold ?? undefined;

        // Build content_items query with optional domain filter
        ...
        const belowThreshold: BelowThresholdItem[] = [];
        ...
        // Process score drops
        const scoreDrops: ScoreDropItem[] = [];
        ...
        // Process freshness transitions
        const freshnessTransitions: FreshnessTransitionItem[] = [];
        ...
        // Quality flags
        const qualityFlags = ((qualityFlagsResult.data ?? []) as QualityFlagNotification[]);

        // Coverage alerts
        const coverageAlerts = ((coverageAlertsResult.data ?? []) as CoverageAlertNotification[]);

        // Process certification warnings
        const certWarnings: CertificationWarning[] = [];
        ...
        const briefingData: QualityBriefingData = {
        ...
        const message = err instanceof Error ? err.message : 'Unknown error';
        return {
          content: [{ type: 'text' as const, text: `Quality briefing failed: ${message}.` }],
          isError: true,
        };
      }
    },
  ...
  server.registerTool(
    ...
    async (args, extra: ToolExtra) => {
      try {
        ...
        const message = err instanceof Error ? err.message : 'Unknown error';
        return {
          content: [{ type: 'text' as const, text: `Quality actions analysis failed: ${message}.` }],
          isError: true,
        };
      }
    },
