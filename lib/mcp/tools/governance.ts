    ...
    async (args, extra: ToolExtra) => {
      try {
        ...
        const [sdRows, qaRows, lifecycleRows] = await Promise.all([
          supabase
            .from('source_documents')
            .select('id, suggested_title, publication_status')
            .in('id', args.item_ids),
          supabase
            .from('q_a_pairs')
        ...
        // SD `content` formerly gated the (retired, DR-130) publish-time
        // auto-classify — no body fetch is needed for source_documents now;
        // only q_a_pairs read `content` (answer_standard) for embedding.
        ...
        for (const itemId of args.item_ids) {
          ...
          try {
            if (args.status === 'publish') {
              ...
              // The former S183 WP1 G2 publish-classify leg (run
              // classifyContent on first publish of draft-created items) was
              // retired with the per-document subject-classification stage
              // (DR-130). Enrichment (entities/embeddings/summary) is the
              // cocoindex pipeline's concern; app-side publish no longer
              // runs a classification pass. Chunking was already removed
              // (ID-56.11): cocoindex is the sole content_chunks writer.
            } else {
    ...
    async (args, extra: ToolExtra) => {
      try {
        ...
        try {
          ...
          // `user_id = auth.uid()`, so the RLS-scoped MCP client is denied
          // every time — and PostgREST resolves that denial in-band as
          // `{ error }` rather than throwing, so the catch below never saw
          // it. Cross-user inserts go through the service client, with the
