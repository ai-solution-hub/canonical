...
/**
 * Mirror of EP8 / Python `should_auto_supersede` filename heuristic.
 * Public via re-export from `lib/ingest/draft-final-heuristic.ts` (T2 sibling)
 * but the orchestrator inlines the simple rule to avoid a build-order coupling
 * between W1 sibling agents.
 */
function detectDraftFinalFromFilename(
  filename: string,
): 'draft' | 'final' | 'unknown' {
...
/**
 * Map the draft/final flag to the canonical `publication_status` value
 ...
 */
function draftFinalToPublicationStatus(
  flag: 'draft' | 'final' | 'unknown',
): 'draft' | 'in_review' {
  if (flag === 'final') return 'in_review';
  return 'draft';
}
