...
export async function extractStructuredContent(
  ...
  // classification family; `title` has no SD column of the same name —
  // original_filename+filename are the nearest analogs). `metadata` was
  // selected but never read in this function — dropped rather than mapped to
  // extraction_metadata. The document BODY is composed from content_chunks /
  // reference_items (id-392 M6 retarget), not a source_documents column.
  const { data: item, error: fetchError } = await supabase
    .from('source_documents')
    .select('id, original_filename, filename, content_type')
    .eq('id', itemId)
    .single();
  ...
  const contentSlice = body.slice(0, MAX_CONTENT_LENGTH);
