...
export const queryKeys = {
  ...
  procurement: {
    ...
    // `forms` sub-namespace REMOVED (ID-145 form-first re-key, DR-038/DR-056):
    // the ID-130 workspace-umbrella-holds-many-forms model it keyed is retired
    // — the item IS the form, so the top-level `procurement.*` keys already
    // key the form directly. Zero consumers remained.
    /**
