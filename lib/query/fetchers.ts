...
// consumer. That status route (and its `fetchContentIngestStatus` fetcher)
// was itself retired at ID-131.24 (G-UPLOAD-GATE): the folder-drop admission
// leg returns its `source_documents.id` synchronously now (DR-020/DR-025
// binding gate, ID-138 {138.13}), so there is no `content_items` row left to
// poll for. The `pipeline_runs` table, its server-side writes, and the LIST
