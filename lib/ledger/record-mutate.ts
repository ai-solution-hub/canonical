 ...
 * ID-148.12 (Option C): re-vendored from v0.5.0 (the prior actual sync point)
 * to v0.10.1 — carries the repurposed nested-initiatives insert/remove arm
 * (INV-13, via the new vendored `initiatives-tree.ts`) PLUS an upstream-native
 * delta unrelated to ID-148 (ID-90.9 U5's create-defaults/nextId/high-water/
...
export function insertSubtasks(
  ...
  for (const raw of subtasks) {
    ...
    let record = withCreateDefaults('subtask', raw as Record<string, unknown>);
