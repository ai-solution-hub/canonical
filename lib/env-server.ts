 ...
 *
 * **Server-only.** The client-side schema (`NEXT_PUBLIC_*`) lives in
 * `lib/env-client.ts` and MUST be imported from there directly — this module
 * pulls Node-only deps and the field names it declares are secrets that must
 * not be substituted into the client bundle.
 *
 ...
 *      this module: `await import('@/lib/env-server')`.
