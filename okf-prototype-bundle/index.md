...
**Status: UNSIGNED PROTOTYPE — pending per-concept owner sign-off ({163.9} interim
gate).**
...
- [Classify Content](tool/classify_content.md) — `classify_content`
  (`lib/mcp/tools/ai.ts`)
- [Find Knowledge](tool/find.md) — `find` (`lib/mcp/tools/search.ts`)
- [Get Content](tool/get.md) — `get` (`lib/mcp/tools/content.ts`)
- [Get Procurement Detail](tool/get_procurement_detail.md) — `get_procurement_detail`
  (`lib/mcp/tools/procurement.ts`)

## navigation (E2 — markdown-page grain)

- [Extend Registry Vendor-In Provenance](navigation/extend-registry-provenance.md) —
  `docs/extend-registry-provenance.md`
