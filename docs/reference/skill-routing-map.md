<!-- Last verified: 29/05/2026 (kh-prod-readiness-S53 ID-18.1 — initial authoring, Option C ratified at S52 close; kh-prod-readiness-S277 ID-23.14 — code-intelligence refactor tilt row added; ID-48.18 — /verify optional runtime-gate conditional added to Frontend — React tilt) -->

# Skill Routing Map

**Last verified:** 29/05/2026
...
| **CI / CD** — GitHub Actions workflows, Vercel config, Coolify/on-prem deploy, knip baseline, PR checks | `diagnose-ci-failures`, `test-driven-development` | `supabase-postgres-best-practices` (if workflow touches migration or schema-parity job); `security-review` (if secrets handling changes) | Do NOT load `vercel-react-best-practices` for pure CI workflow changes — Vercel build config is not a React surface. Do NOT use `playwright-best-practices` for workflow YAML edits. | CI job parallelisation, knip baseline update, Coolify/on-prem deploy fix, GitHub Environment config |
...
| **Frontend — React** — Next.js App Router pages, React components, hooks, TanStack Query, Tailwind / design tokens | `test-driven-development`, `incremental-implementation` | `playwright-best-practices` (if change has E2E coverage); `security-review` (if auth-gated route added); `code-simplification` (end-of-task pass); `/verify` (optional end-of-task additive runtime gate — launches the app via the `run-knowledge-hub` recipe to confirm a user-facing change actually RUNS; does NOT replace the Checker) | Do NOT use raw Tailwind colour classes — semantic tokens only (`app/globals.css`). Do NOT use `astro` skill for React surface. Do NOT use SWR or raw `fetch` in hooks — TanStack Query only. Do NOT treat `/verify` as a Checker substitute — it is an additive runtime gate run AFTER a Checker PASS, never instead of one. | New page route, component rebuild, hook refactor, design-token update, TanStack Query migration |
...
| **Data pipeline** — Python `kb_pipeline/`, ingestion CLIs, batch scripts, Coolify/on-prem pipeline jobs, `pipeline_runs` records | `test-driven-development` | `supabase-postgres-best-practices` (if pipeline writes schema); `diagnose-ci-failures` (if the Coolify/on-prem deploy CI job is red); `security-review` (if service-account credentials touched) | Do NOT use `PYTHONUNBUFFERED=0` — pipeline output becomes invisible (CLAUDE.md §General). Do NOT use `json.dumps()` for Supabase metadata — pass dict directly. Do NOT use `Document(path)` from python-docx directly — use `open_document_safe()`. | Ingestion pipeline, batch reclassify, Coolify/on-prem deploy job, `pipeline_runs` logging, taxonomy snapshot refresh |
...
| **Security / RLS** — RLS policy authoring, `get_user_role()`, SECURITY DEFINER audit, api-grant coverage, snyk scan | `security-review`, `supabase-postgres-best-practices`, `test-driven-development` | `diagnose-ci-failures` (if `api-grant-guard.yml` or `supabase-advisors.yml` CI job is red) | Do NOT use `--scan-all-users` with snyk-agent-scan — it dumps env-var values verbatim in JSON (CLAUDE.md §Plugin/MCP). Do NOT expose new `public.*()` helpers to the Data API — `public` is unexposed by design (ID-115 PGRST106 boundary); surface via the `api` schema and regenerate with `generate-api-views.ts`. Do NOT grant anon SELECT on `user_roles`. | OPS-43 SECDEF→INVOKER batch, advisor lint fix, RLS policy tightening, api-view coverage regen |
