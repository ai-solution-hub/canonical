...
- **Keep references one level deep** - All reference files should link directly from
  SKILL.md
...
The description field enables skill discovery. the agent uses it to decide when to load
the skill.
...
Context window is shared across all skills, conversation history, and the system prompt.
Every token matters.
...
and other content. To extract text from a PDF, you'll need to use a library. There are
many libraries available for PDF processing, but we recommend pdfplumber because it's easy
to use and handles most cases well. First, you'll need to install it using pip. Then you
...
\`\`\`rust if FeatureFlag::YourFeature.is_enabled() { // new behavior } else { // old
behavior (dead code) } \`\`\`

### After:

\`\`\`rust // new behavior (unconditionally enabled) \`\`\`
```
...
\`\`\`rust if FeatureFlag::YourFeatureName.is_enabled() { // feature-gated behavior }
\`\`\`
```
...
Not every skill needs an Overview, Best Practices, and Examples section. Use only what
adds value:
...
Keep iterations focused. Don't add content preemptively—only add what's proven necessary
through real usage.
