...
// id-417 / DR-130: the `domain` filter and the `unclassified` sentinel filter
// retired with the subject-taxonomy axis (source columns dropped).
...
// id-417 / DR-130: `confidence_asc` retired with classification_confidence.
export type ReviewQueueSortField = 'created_at' | 'quality_score_asc';
...
export interface ReviewStatsResponse {
  ...
  // id-417 / DR-130: `unclassified_coverage` and `by_domain` retired with the
  // subject-taxonomy axis (sentinel columns + record_lifecycle.domain dropped;
  // get_review_breakdown_stats no longer emits a by_domain key).
