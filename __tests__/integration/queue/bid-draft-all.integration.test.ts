...
async function createTestBid(opts: {
  ...
  return { bidId: bid.id, questionIds };
}
...
function buildPostRequest(
  ...
  return new Request(`http://localhost/api/bids/${bidId}/responses/draft-all`, {
    method: 'POST',
...
describeIfEnv('AC-6 — bid not in draftable state', () => {
  ...
  it('AC-6 (handler-level): direct envelope insert + flip bid to matching → cron tick → status=failed, error_message mentions bid_not_draftable', async () => {
    ...
    const idempotencyKey = `bid_draft_all:${bidId}:2026-05-05:${TEST_PREFIX}_AC6h`;
    const envelope: Json = {
      envelope_version: 1,
      auth_context: {
...
describeIfEnv('AC-7 — bid with 0 questions → permanent failure', () => {
  it('AC-7: enqueue + cron tick → status=failed, error_message=no_questions_in_bid', async () => {
    const { bidId } = await createTestBid({
      status: 'drafting',
      zeroQuestions: true,
    });

    const idempotencyKey = `bid_draft_all:${bidId}:2026-05-05:${TEST_PREFIX}_AC7`;
    const envelope: Json = {
      envelope_version: 1,
      auth_context: {
        user_id: EDITOR_USER_ID,
