...
vi.mock('@/lib/anthropic', () => ({
  ...
  estimateCost: vi.fn((model: string, usage: { input_tokens: number; output_tokens: number }) => {
...
const testContent: DraftableContent[] = [
  {
    ...
    content: '<p>We implement ISO 27001 controls across all systems. Data is encrypted at rest and in transit using AES-256.</p>',
    content_type: 'policy',
    ai_summary: 'Comprehensive data security policy covering ISO 27001, encryption, and access controls.',
  },
  {
    id: 'c-002',
    title: 'GDPR Compliance Framework',
    content: '<p>Our GDPR framework includes data mapping, DPIAs, and a dedicated Data Protection Officer.</p>',
    content_type: 'article',
    ai_summary: 'GDPR compliance framework with data mapping, impact assessments, and DPO.',
  },
];

/** Build a mock Anthropic API response for structured output (Pass 1 / Pass 3) */
function mockStructuredResponse(jsonContent: object, inputTokens = 200, outputTokens = 100) {
  return {
    content: [
      {
        type: 'text',
...
function mockCitedResponse(
  text: string,
  citations: Array<{ source: string; title: string; cited_text: string; search_result_index: number }>,
  inputTokens = 1000,
  outputTokens = 500,
) {
  return {
...
const defaultAnalysis: QuestionAnalysis = {
  ...
  response_structure: {
    suggested_headings: ['Introduction', 'Data Security', 'GDPR Compliance', 'Conclusion'],
    word_allocation: [
...
describe('Bid Drafting Pipeline', () => {
  ...
  describe('Pass 1: analyseQuestion', () => {
    it('returns parsed analysis from Anthropic structured output', async () => {
      mockCreate.mockResolvedValueOnce(mockStructuredResponse(defaultAnalysis, 150, 80));
      ...
      expect(result.analysis.key_points_to_cover).toEqual(['ISO 27001', 'Encryption', 'Data mapping', 'DPO']);
      expect(result.analysis.response_structure.suggested_headings).toHaveLength(4);
    });

    it('accumulates token usage and cost', async () => {
      mockCreate.mockResolvedValueOnce(mockStructuredResponse(defaultAnalysis, 300, 150));
    ...
    it('falls back to default analysis when JSON parsing fails', async () => {
      ...
      expect(result.analysis.primary_topic).toBe(testQuestion.question_text.slice(0, 100));
      expect(result.analysis.tone).toBe('formal');
    });
    ...
    it('handles null word limit gracefully', async () => {
      ...
      const noLimitQuestion = { ...testQuestion, word_limit: null, section_name: null };
  ...
  describe('Pass 2: draftResponse', () => {
    it('returns response text with extracted citations', async () => {
      mockCreate.mockResolvedValueOnce(
        mockCitedResponse(
          ...
          [
            { source: '/item/c-001', title: 'Data Security Policy', cited_text: 'ISO 27001 controls', search_result_index: 0 },
            { source: '/item/c-002', title: 'GDPR Compliance Framework', cited_text: 'dedicated Data Protection Officer', search_result_index: 1 },
          ],
        ),
      );

      const result = await draftResponse(testQuestion, testContent, defaultAnalysis);
    ...
    it('uses the correct model tier', async () => {
      ...
      await draftResponse(testQuestion, testContent, defaultAnalysis, 'drafting');
    ...
    it('includes regeneration instructions when provided', async () => {
      mockCreate.mockResolvedValueOnce(mockCitedResponse('Updated response.', []));

      await draftResponse(testQuestion, testContent, defaultAnalysis, 'drafting', 'Add more detail about encryption standards');

      const call = mockCreate.mock.calls[0][0];
      const systemText = call.system[0].text;
      expect(systemText).toContain('ADDITIONAL INSTRUCTIONS FROM REVIEWER');
      expect(systemText).toContain('Add more detail about encryption standards');
    });
    ...
    it('builds search result blocks with cache_control on last 3 only', async () => {
      ...
      const fourItems: DraftableContent[] = [
        ...testContent,
        { id: 'c-003', title: 'Item 3', content: 'Content 3', content_type: 'article', ai_summary: 'Summary 3' },
        { id: 'c-004', title: 'Item 4', content: 'Content 4', content_type: 'article', ai_summary: 'Summary 4' },
      ];
      ...
      const searchBlocks = userContent.filter((b: { type: string }) => b.type === 'search_result');
  ...
  describe('Pass 3: Deterministic quality checks', () => {
    it('detects word count over limit', () => {
      ...
      const { wordCount, issues } = runDeterministicChecks(longText, [], { question_text: 'Q', word_limit: 500 }, 1);

      expect(wordCount).toBe(600);
      expect(issues.some((i) => i.type === 'word_limit' && i.severity === 'error')).toBe(true);
    });

    it('warns when word count is below 70% of limit', () => {
      const shortText = '<p>' + 'word '.repeat(100) + '</p>';
      const { issues } = runDeterministicChecks(shortText, [], { question_text: 'Q', word_limit: 500 }, 1);

      expect(issues.some((i) => i.type === 'word_limit' && i.severity === 'warning')).toBe(true);
    });

    it('warns when no citations despite available content', () => {
      const text = '<p>' + 'word '.repeat(400) + '</p>';
      const { issues } = runDeterministicChecks(text, [], { question_text: 'Q', word_limit: 500 }, 3);

      expect(issues.some((i) => i.type === 'unsupported_claim')).toBe(true);
    });

    it('reports empty response as error', () => {
      const { wordCount, issues } = runDeterministicChecks('', [], { question_text: 'Q', word_limit: 500 }, 1);

      expect(wordCount).toBe(0);
      expect(issues.some((i) => i.type === 'missing_section' && i.severity === 'error')).toBe(true);
    });

    it('passes when word count is within limit with citations', () => {
      const text = '<p>' + 'word '.repeat(450) + '</p>';
      const citations = [{ cited_text: 'test', source_index: 0, source_id: 'c-001', source_title: 'Title', source_url: '/item/c-001', start_block_index: 0, end_block_index: 0 }];
      const { issues } = runDeterministicChecks(text, citations, { question_text: 'Q', word_limit: 500 }, 1);
    ...
    it('handles null word limit (no compliance check)', () => {
      ...
      const { issues } = runDeterministicChecks(text, [], { question_text: 'Q', word_limit: null }, 0);
  ...
  describe('runDraftingPipeline (full three-pass)', () => {
    beforeEach(() => {
      ...
      mockCreate.mockResolvedValueOnce(
        mockCitedResponse(
          ...
          [
            { source: '/item/c-001', title: 'Data Security Policy', cited_text: 'ISO 27001', search_result_index: 0 },
            { source: '/item/c-002', title: 'GDPR Compliance Framework', cited_text: 'dedicated DPO', search_result_index: 1 },
          ],
          1500,
          600,
        ),
    ...
    it('includes quality data in metadata', async () => {
      ...
      expect(result.metadata.quality_data.suggestions).toContain('Consider adding a specific case study example');
    });
    ...
    it('maps source content IDs correctly', async () => {
      ...
      expect(result.metadata.citations_data.source_content_ids).toEqual(['c-001', 'c-002']);
    });
    ...
    it('passes regeneration instructions through to Pass 2', async () => {
      ...
      mockCreate
        .mockResolvedValueOnce(mockStructuredResponse(defaultAnalysis, 200, 100))
        .mockResolvedValueOnce(mockCitedResponse('Regenerated response.', [], 1500, 600))
        .mockResolvedValueOnce(mockStructuredResponse(defaultQualityResult, 300, 80));
      ...
      expect(result.metadata.ai_metadata.regeneration_instructions).toBe('Focus more on encryption standards');

      // Verify Pass 2 received the instructions
      const pass2Call = mockCreate.mock.calls[1][0];
      expect(pass2Call.system[0].text).toContain('Focus more on encryption standards');
    });
    ...
    it('handles empty matched content (no KB sources)', async () => {
      ...
      mockCreate
        ...
        .mockResolvedValueOnce(mockCitedResponse('A response without KB backing.', [], 500, 200))
        .mockResolvedValueOnce(mockStructuredResponse(defaultQualityResult, 150, 40));
