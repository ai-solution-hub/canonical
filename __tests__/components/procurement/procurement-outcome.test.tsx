/**
 * ProcurementOutcomeDialog Component Tests
 *
...
const defaultProps = {
  open: true,
  onOpenChange: vi.fn(),
  procurementId: 'bid-123',
  procurementName: 'Test Procurement Alpha',
  onOutcomeRecorded: vi.fn(),
};

function renderDialog(overrides: Partial<typeof defaultProps> = {}) {
  const props = { ...defaultProps, ...overrides };
  return render(<ProcurementOutcomeDialog {...props} />);
}
...
describe('ProcurementOutcomeDialog', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('renders the dialog when open is true', () => {
    renderDialog();
    expect(screen.getByText('Record Procurement Outcome')).toBeInTheDocument();
  });
  ...
  it('shows the bid name in the description', () => {
    renderDialog();
    expect(screen.getByText('Test Procurement Alpha')).toBeInTheDocument();
  });
  ...
  it('submits the outcome and calls onOutcomeRecorded on success', async () => {
    ...
    await waitFor(() => {
      expect(globalThis.fetch).toHaveBeenCalledWith(
        '/api/procurement/bid-123/outcome',
    ...
    await waitFor(() => {
      expect(mockToast.success).toHaveBeenCalledWith(
        'Procurement outcome recorded: Won',
        { duration: 3000 },
      );
