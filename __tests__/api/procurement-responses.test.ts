/**
 * Procurement response API route tests.
 *
...
describe('Procurement Responses API', () => {
  beforeEach(() => {
    resetMocks();
  });
  ...
  describe('GET /api/bids/:id/responses/:rId', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const request = createTestRequest(
        `/api/procurement/${BID_UUID}/responses/${RESPONSE_UUID}`,
      );
    ...
    it('returns 400 for invalid bid UUID', async () => {
      const request = createTestRequest(
        `/api/procurement/not-a-uuid/responses/${RESPONSE_UUID}`,
      );
    ...
    it('returns 400 for invalid response UUID', async () => {
      const request = createTestRequest(
        `/api/procurement/${BID_UUID}/responses/bad-id`,
      );
  ...
  describe('PATCH /api/bids/:id/responses/:rId', () => {
    ...
    it('returns 400 for invalid UUID in params', async () => {
      ...
      const request = createTestRequest(
        `/api/procurement/not-valid/responses/${RESPONSE_UUID}`,
        { method: 'PATCH', body: { response_text: 'Updated' } },
      );
