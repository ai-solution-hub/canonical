...
describe('Bid Export API', () => {
  ...
  describe('POST /api/bids/:id/export/docx', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const request = createTestRequest(`/api/bids/${BID_UUID}/export/docx`, {
        method: 'POST',
        body: {},
      });
    ...
    it('returns 400 for invalid bid ID', async () => {
      const request = createTestRequest('/api/bids/not-a-uuid/export/docx', {
        method: 'POST',
        body: {},
      });
    ...
    it('returns correct Content-Disposition header for DOCX', async () => {
      configureBidWithQuestions('My Test Bid!');
  ...
  describe('POST /api/bids/:id/export/xlsx', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const request = createTestRequest(`/api/bids/${BID_UUID}/export/xlsx`, {
        method: 'POST',
        body: {},
      });
    ...
    it('returns 400 for invalid bid ID', async () => {
      const request = createTestRequest('/api/bids/bad-uuid/export/xlsx', {
        method: 'POST',
        body: {},
      });
    ...
    it('handles bid with no questions gracefully', async () => {
      ...
      // fetchBidExportData returns 404 for empty questions
      expect(response.status).toBe(404);
