...
describe('POST /api/items/batch', () => {
  ...
  // S446): the per-item exact-hash soft-block stamping (backed by the
  // now-DROPped find_exact_duplicates RPC) was removed. Every item is
  // always stamped 'clean'; `skip_dedup` is a no-op for admin and
  // non-admin callers alike.
  // ─────────────────────────────────────────────────────────────────────

  describe('dedup retirement (ID-131.15) — always-clean stamping', () => {
    /** Content must exceed 50 chars — mirrors the legacy hash-check floor,
     * kept only so these fixtures resemble realistic batch payloads. */
    ...
    it('stamps dedup_status=clean for every item — no on-ingest exact-hash check runs', async () => {
      configureRole(mockSupabase, 'editor');
      ...
      for (const call of insertCalls) {
        ...
        if (insertData.content_type === 'q_a_pair') {
          expect(insertData.dedup_status).toBe('clean');
        }
      }
    });

    it('skip_dedup=true is a no-op — items still stamp clean (admin or non-admin)', async () => {
      configureRole(mockSupabase, 'admin');
