...
// Pattern mirrors lib/bid/bid-queries.ts:70 — value passed as a named property
// in an object literal that is itself an argument to a call.
//
// origin: const bidIds (line 12, column 9)
// hop 2: argument hop — execute('fn', { p_project_ids: bidIds }) call site
...
export async function fetchStats(client: QueryClient) {
  const bidIds = [1, 2, 3];
  await client.execute('get_bid_question_stats_batch', { p_project_ids: bidIds });
}
