...
describe('bid validation schemas', () => {
  describe('BidCreateBodySchema', () => {
    it('accepts valid full input', () => {
      const result = BidCreateBodySchema.safeParse({
    ...
    it('trims name whitespace', () => {
      ...
      expect(result.success).toBe(true);
      if (result.success) {
        expect(result.data.name).toBe('Test Bid');
        expect(result.data.buyer).toBe('Test Buyer');
      }
    });
  });

  describe('BidUpdateBodySchema', () => {
    it('accepts partial update with name only', () => {
      const result = BidUpdateBodySchema.safeParse({ name: 'Updated Name' });
      expect(result.success).toBe(true);
    });

    it('accepts status transition', () => {
      const result = BidUpdateBodySchema.safeParse({ status: 'drafting' });
      expect(result.success).toBe(true);
    });
    ...
    it('rejects invalid outcome', () => {
      const result = BidUpdateBodySchema.safeParse({ outcome: 'tied' });
      expect(result.success).toBe(false);
    });
