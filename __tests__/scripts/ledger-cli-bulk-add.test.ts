...
describe('add-subtasks — bulk JSON-array create in ONE scoped multi-splice (ID-65.6)', () => {
  ...
  // ── {35.16} add-many record-set gate on the BULK write ───────────────────────
  // ID-90.22 R1a: the two serialise-side induction tests (DROP / DUPLICATE via a
  // `scopedSpliceSerialise` namespace stub) are RETIRED. The bulk write now
  // routes through the SERVER TRANSPORT, where the {35.16} add-many record-set
  // gate runs server-side on the bytes the SERVER serialises — the in-process
  // `scopedSpliceSerialise` stub no longer sits on the write path, so a
  // serialise-side drop/duplicate cannot be induced from this process. The
  // server-side gate is covered by task-view's own suite (U11). The OBSERVABLE
  // record-set-violation envelope (exit-worthy, NO bytes written) is asserted in
  // ledger-cli-record-set.test.ts via the flag-ON LOCAL `--whole-file` path
  // (GAP-2a). The HONEST positive coverage of the bulk happy-path + auto-id
  // assignment + per-batch reject (invalid-depends / invalid-id / unknown-task)
  // remains above, all exercised over the transport.
});
