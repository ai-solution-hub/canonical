...
vi.mock('next/navigation', () => ({
  ...
  usePathname: () => `/procurement/${hoistedBidId}`,
}));
...
function mockFetchSuccess(overrides?: {
  ...
  const procurementStatus = overrides?.procurementStatus ?? 200;

  mockFetch.mockImplementation((url: string, init?: RequestInit) => {
    // Procurement detail GET
    if (
      url === `/api/procurement/${TEST_BID_ID}` &&
      (!init?.method || init?.method === 'GET')
    ) {
      if (procurementStatus === 404) {
    ...
    // Questions GET
    if (
      url === `/api/procurement/${TEST_BID_ID}/questions` &&
      (!init?.method || init?.method === 'GET')
    ) {
      return Promise.resolve({
        ok: true,
    ...
    // Match questions
    if (
      url === `/api/procurement/${TEST_BID_ID}/questions/match` &&
      init?.method === 'POST'
    ) {
      return Promise.resolve({
        ok: true,
    ...
    if (
      url === `/api/procurement/${TEST_BID_ID}/responses/draft-all` &&
      init?.method === 'POST'
    ) {
      return Promise.resolve({
        ok: true,
...
describe('useFormActions (TanStack Query)', () => {
  ...
  // ─── 2. Procurement data populated from query ─────────────────────────────────
  ...
  it('populates bid data from query', async () => {
    ...
    expect(result.current.bid?.title).toBe('Test Procurement');
  ...
  it('rejects invalid transitions with toast error', async () => {
    ...
    await act(async () => {
      result.current.handleStatusTransition('won' as ProcurementWorkflowState);
    });
  ...
  it('handleStatusTransition shows error toast on API failure', async () => {
    ...
    vi.mocked(global.fetch).mockImplementation(async (url, init) => {
      if (
        String(url) === `/api/procurement/${TEST_BID_ID}` &&
        init?.method === 'PATCH'
      ) {
        return {
          ok: false,
  ...
  it('handleDeleteConfirmed deletes and redirects', async () => {
    ...
    await waitFor(() => {
      expect(toast.success).toHaveBeenCalledWith('Procurement deleted');
      expect(mockPush).toHaveBeenCalledWith('/procurement');
    });
  });
  ...
  it('handleMatchQuestions calls POST and shows success', async () => {
    ...
    await waitFor(() => {
      expect(mockFetch).toHaveBeenCalledWith(
        `/api/procurement/${TEST_BID_ID}/questions/match`,
  ...
  it('handleUploadComplete sets extracted questions and navigates to questions tab via URL', async () => {
    ...
    expect(mockReplace).toHaveBeenCalledWith(
      `/procurement/${TEST_BID_ID}?tab=questions`,
    );
  ...
  it('transitioning is true while transition mutation is pending', async () => {
    ...
    mockFetch.mockImplementation((url: string, init?: RequestInit) => {
      ...
      if (url === `/api/procurement/${TEST_BID_ID}/questions`) {
        return Promise.resolve({
          ok: true,
          json: async () => MOCK_QUESTIONS_RESPONSE,
        });
  ...
  it('returns all expected properties', async () => {
    ...
    // Data refresh
    expect(result.current).toHaveProperty('fetchProcurement');
    expect(result.current).toHaveProperty('fetchQuestions');

    // Computed
    expect(result.current).toHaveProperty('metadata');
    expect(result.current).toHaveProperty('procurementStatus');
  ...
  describe('URL-synced tab state (P1-5)', () => {
    ...
    it('setActiveTab to overview removes ?tab= param for clean URLs', async () => {
      ...
      expect(mockReplace).toHaveBeenCalledWith(`/procurement/${TEST_BID_ID}`);
    });
