...
interface PdfDocumentProps {
  ...
  /**
   * When false, keyboard navigation listeners are not attached. The inline
   * reader leaves this on; the dialog viewer gates it on the dialog being open
   * so background documents do not capture arrow/zoom keys.
   * @default true
   */
  keyboardNavEnabled?: boolean;
  /** Tailwind padding class for the scrollable PDF content area. */
...
/**
 * Shared PDF engine: signed-URL resolution, page/zoom state, keyboard
 * navigation, toolbar, and the `react-pdf` `<Document>`/`<Page>` render. Renders
 * the loader, error, and content paths internally. Consumers wrap it (inline or
 * in a Dialog) and own their chrome — see `pdf-reader-view.tsx` and
 * `pdf-viewer.tsx`.
 */
export function PdfDocument({
  ...
  }, [keyboardNavEnabled, currentPage, goToPage, zoomIn, zoomOut]);
  ...
  return (
    <div ref={containerRef} className="flex h-full flex-col">
      {/* Toolbar */}
      <div
        className={`flex flex-wrap items-center gap-2 border-b border-border py-1.5 ${toolbarPaddingClassName}`}
      >
        {/* Page navigation */}
      ...
      {/* PDF content */}
      <div
        className={`flex-1 overflow-auto bg-muted/30 ${contentPaddingClassName}`}
      >
