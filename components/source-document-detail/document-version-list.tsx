...
/**
 * DocumentVersionList — id-135 {135.15}, TECH.md §3 BI-25/BI-26, §4, RD-4.
 *
 * A THIN presentational map of `get_document_version_chain` rows (the
 * shipped id-117 RPC, reused as-is) driven by its OWN independent
 * `useDocumentVersions` {135.13} query — mirroring the BI-30 pattern its
...
const EMPTY_VERSIONS: DocumentVersionRow[] = [];
...
export function DocumentVersionList({ documentId }: DocumentVersionListProps) {
  const { data, isLoading, isError, refetch } = useDocumentVersions(documentId);
...
function VersionRows({ versions }: { versions: DocumentVersionRow[] }) {
