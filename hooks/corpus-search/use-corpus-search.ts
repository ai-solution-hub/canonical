...
//
// `RawCorpusSearchRow` is the verified `hybrid_search` RPC row shape (21
// columns; `supabase/migrations/20260702120000_id131_search_rpcs.sql`,
...
//
// Two confirmed gaps against the TECH §5 target `CorpusSearchResult` union,
// degraded gracefully rather than fabricated (surfaced in the {135.6}
// journal for the Orchestrator to route towards {131.11}/{131.19}):
//   - `hybrid_search` has no `scope_tag` column, so an `answer` result's
//     `scopeTags` cannot be populated today — defaults to `[]`.
//   - it has no reference `source_url` column, so a `reference` result's
//     `sourceUrl` cannot be populated today — defaults to `null`.
// ---------------------------------------------------------------------------
...
interface RawCorpusSearchRow {
  ...
  /**
   * Confirmed drift from the BI-14/AAT-2 "content_type carries owner_kind"
   * assumption: `hybrid_search`'s arm 4 (q_a_pair/content_chunk/reference_item)
   * emits the literal owner_kind ('q_a_pair' | 'content_chunk' |
   * 'reference_item'), but its source_documents arm emits
   * `sd.content_type` verbatim — the DOCUMENT'S OWN taxonomy content-type
   * value (e.g. 'guidance'), not the literal 'source_document'.
   * `resolveCorpusKind` below treats every value other than the two
...
/** Owner_kind literals `hybrid_search` emits verbatim for these two arms. */
function resolveCorpusKind(contentType: string): CorpusKind {
  if (contentType === 'q_a_pair') return 'answer';
  if (contentType === 'reference_item') return 'reference';
  // Everything else — the literal 'content_chunk' owner_kind (BI-12:
  // content_chunk hits collapse to their source_document) and the
  // source_documents arm's own taxonomy content_type value — is `document`.
  return 'document';
}

function toCorpusSearchResult(row: RawCorpusSearchRow): CorpusSearchResult {
  const kind = resolveCorpusKind(row.content_type);
  ...
  if (kind === 'answer') {
    return {
      ...
      scopeTags: [], // See the RawCorpusSearchRow gap note above.
  ...
  if (kind === 'reference') {
    return {
      ...
      kind: 'reference',
      title,
      sourceUrl: null, // See the RawCorpusSearchRow gap note above.
    };
  }
...
export function useCorpusSearch(): UseCorpusSearchReturn {
  ...
  >({
    ...
    queryFn: async ({ pageParam, signal }) => {
      ...
      // Defensive client-side narrow (BI-15) — `hybrid_search` has no
      // grain-narrow param today; filtering (never reordering) keeps the
      // "narrows only" contract true now, ahead of server-side narrowing.
      const narrowed = kind ? mapped.filter((r) => r.kind === kind) : mapped;
