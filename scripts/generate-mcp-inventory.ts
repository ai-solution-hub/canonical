...
function generateMarkdown(inventory: InventoryJSON): string {
  ...
  lines.push('<!-- AUTO-GENERATED by scripts/generate-mcp-inventory.ts -- do not edit manually -->');
  lines.push('');
  ...
  lines.push('## Summary');
  lines.push('');
  lines.push(`- **Tools:** ${inventory.summary.tool_count} (${inventory.summary.read_only_tools} read-only, ${inventory.summary.write_tools} write, ${inventory.summary.destructive_tools} destructive)`);
  lines.push(`- **Resources:** ${inventory.summary.resource_count} (${inventory.summary.template_resources} templates, ${inventory.summary.app_resources} app resources)`);
  ...
  lines.push('## Tools');
  lines.push('');
  lines.push('| # | Name | Title | Category | Params | Read-only | Destructive |');
  lines.push('|---|------|-------|----------|--------|-----------|-------------|');

  for (const tool of inventory.tools) {
    const paramStr = tool.input_params.length === 0
      ? '(none)'
      : tool.input_params
  ...
  for (let i = 0; i < inventory.resources.length; i++) {
    ...
    const resourceType = r.is_app_resource ? 'App' : r.is_template ? 'Template' : 'Static';
    lines.push(
  ...
  for (let i = 0; i < inventory.prompts.length; i++) {
    ...
    const argsStr = p.args.length === 0
      ? '(none)'
      : p.args.map((a) => a.name).join(', ');
...
function validateFixtures(inventory: InventoryJSON): ValidationResult {
  ...
  for (const name of inventoryToolNames) {
    if (!fixtureToolSet.has(name)) {
      issues.push(`Tool in code but missing from CANONICAL_TOOL_NAMES: ${name}`);
    }
  }
  ...
  for (const name of fixtureToolNames) {
    if (!inventoryToolNames.has(name)) {
      issues.push(`Tool in CANONICAL_TOOL_NAMES but not found in code: ${name}`);
    }
  }
  ...
  for (const tool of inventory.tools) {
    ...
    if (isReadOnly && fixtureWrite.has(tool.name)) {
      issues.push(`Tool ${tool.name} is read-only in code but in WRITE_TOOLS fixture`);
    }
    if (!isReadOnly && fixtureWrite.size > 0 && fixtureReadOnly.has(tool.name)) {
      issues.push(`Tool ${tool.name} is write in code but in READ_ONLY_TOOLS fixture`);
    }
  }
  ...
  for (const resource of inventory.resources) {
    if (resource.is_template) {
      if (!fixtureTemplateUris.has(resource.uri)) {
        issues.push(`Template resource in code but missing from RESOURCE_TEMPLATE_URIS: ${resource.uri}`);
      }
    } else {
      if (!fixtureStaticUris.has(resource.uri)) {
        issues.push(`Static resource in code but missing from STATIC_RESOURCE_URIS: ${resource.uri}`);
      }
    }
  }
  ...
  for (const uri of fixtureTemplateUris) {
    if (!codeTemplateUris.has(uri)) {
      issues.push(`URI in RESOURCE_TEMPLATE_URIS but not found in code: ${uri}`);
    }
  }
  ...
  for (const name of codePromptNames) {
    if (!fixturePromptNames.has(name)) {
      issues.push(`Prompt in code but missing from CANONICAL_PROMPT_NAMES: ${name}`);
    }
  }
  for (const name of fixturePromptNames) {
    if (!codePromptNames.has(name)) {
      issues.push(`Prompt in CANONICAL_PROMPT_NAMES but not found in code: ${name}`);
    }
  }
...
function generateIndexHeader(inventory: InventoryJSON): string {
  ...
  lines.push(' *');
  lines.push(` * Registers ${inventory.summary.tool_count} tools across ${inventory.summary.category_files} category files:`);
  ...
  lines.push(' *');
  lines.push(' * All tools use per-user Supabase clients via extra.authInfo so that');
  lines.push(' * RLS policies are applied based on the authenticated user.');
  lines.push(' *');
  lines.push(' * Tool naming: names intentionally omit a service prefix (e.g. kb_). The');
  lines.push(' * Knowledge Hub MCP server is designed as a single-purpose connector --');
  lines.push(" * users won't have multiple KB servers. Adding prefixes would make names");
  lines.push(' * unnecessarily verbose for Claude. Revisit if multi-server scenarios arise.');
  lines.push(' */');
...
function main(): void {
  ...
  if (emptyTitles.length > 0) {
    console.warn(`Warning: ${emptyTitles.length} tools have empty titles: ${emptyTitles.map((t) => t.name).join(', ')}`);
  }
  ...
  if (options.dryRun) {
    ...
    console.log(`  docs/generated/mcp-inventory.json (${jsonContent.length} bytes)`);
    if (!options.jsonOnly) {
      console.log(`  docs/generated/mcp-inventory.md (${markdownContent.length} bytes)`);
    }
    console.log('');
    console.log(`Summary: ${inventory.summary.tool_count} tools, ${inventory.summary.resource_count} resources, ${inventory.summary.prompt_count} prompts`);
    console.log(`  Read-only: ${inventory.summary.read_only_tools}, Write: ${inventory.summary.write_tools}, Destructive: ${inventory.summary.destructive_tools}`);
    console.log(`  App tools: ${inventory.summary.app_tools}, App resources: ${inventory.summary.app_resources}`);
    return;
  }
