...
// ---------------------------------------------------------------------------
// 1. Search Tools — PARKED with the O1 rebuild.
//    FC-01..07 drove the consolidated `find` entry, deleted in the S530
//    id-417 wave. The O1 find/answer outcome remains ratified and is
//    RETIRED_PENDING_REBUILD (fixtures.ts) — the id-71 successor surface
//    owns the rebuild, and the search FC checks return with it.
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// 4. Procurement Tools — REMOVED with their subjects.
//    FC-30..32 drove list_active_procurement / get_procurement_detail /
//    get_form_question, all deleted in the S530 id-417 wave (procurement
//    family retired wholesale; not in RETIRED_PENDING_REBUILD_TOOLS).
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// 6. Entity Tools (FC-50)
// ---------------------------------------------------------------------------
...
async function runEntityToolChecks(
  ...
  // FC-51 (get_content_effectiveness) — REMOVED with its subject: the tool
  // died in the S530 id-417 wave with the search/procurement families and is
  // NOT in RETIRED_PENDING_REBUILD_TOOLS (no successor owed).
}

// ---------------------------------------------------------------------------
// 8. App/Template Tools — REMOVED with their subjects.
//    FC-70..75 drove show_coverage_matrix / show_procurement_dashboard /
//    list_templates / get_template_coverage / get_template_gaps /
//    show_reorient_me — all deleted in the S530 id-417 wave (templates/apps
//    families; intelligence-feed is the sole surviving MCP app and has no
//    show_* trigger tool). The old checks were passing against the
//    'Tool not found' error text — a false green, not coverage.
// ---------------------------------------------------------------------------
...
async function runHeadlessCompleteEnumerationChecks(
  ...
  // FC-92 (O1 find/answer live drive) — PARKED with the O1 rebuild, mirroring
  // the FC-93/94/95 (O4/O6) drives removed when their tools were deleted:
  // `find` died in the S530 id-417 wave and is RETIRED_PENDING_REBUILD
  // (fixtures.ts, id-71). FC-90/91 above keep asserting the declarative
  // contract set; the live O1 drive returns with the successor tool.
...
async function main(): Promise<void> {
  ...
  try {
    // Search (FC-01..07), procurement (FC-30..32) and app/template
    // (FC-70..75) sections are retired/parked with their subject tools —
    // see the section notes above.
...
// The check logic above (runEntityToolChecks, runGuideToolChecks, etc.) is
