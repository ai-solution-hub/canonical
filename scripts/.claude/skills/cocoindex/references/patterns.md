...
**Use case**: Transform files from one format to another (e.g., Markdown -> HTML, PDF ->
Markdown)
...
- `mount_each()` -- One component per file; keys from `files.items()` become component
  subpaths
...
- `Annotated[NDArray, EMBEDDER]` -- Vector annotation uses ContextKey for auto dimension
  inference
...
- `kafka.topic_as_map()` returns a `LiveMapFeed` -- `mount_each()` auto-detects for live
  mode
...
- `detect_change=True` -- Opt in to invalidate memos when value changes (use for
  models/config affecting output)
