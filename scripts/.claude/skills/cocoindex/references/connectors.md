...
Target connectors take a `ContextKey` (not the connection object directly) as their first
argument. This enables CocoIndex to track the identity of the target across runs.
...
**Capabilities**: Target only | **Vector**: Native | **Features**: Vector indexes,
inverted indexes, Stream Load
