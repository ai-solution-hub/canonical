 ...
 */
export function parseCanonicalTaxonomy(
  filePath: string,
  ...
  const taxonomySection = content.substring(
  ...
  while ((domainMatch = domainRegex.exec(taxonomySection)) !== null) {
    ...
    const domainEnd = nextDomainMatch
      ? nextDomainMatch.index
      : taxonomySection.length;
