...
def main():
    ...
    parser.add_argument(
        ...
        help="No-op smoke test: import + env-validation only, exit 0 (no DB writes)."
    )
    parser.add_argument(
    ...
    # No-op smoke test: validates env-mount + import chain, exits 0
    # without DB writes. Reachable via local dev / manual invocation.
    if args.smoke_test:
