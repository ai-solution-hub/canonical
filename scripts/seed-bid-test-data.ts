...
async function main() {
  console.log('Bid Test Data Seed Script');
  console.log('========================\n');
    ...
    if (clean) {
      ...
      if (!dryRun) {
        ...
        const { error: bidDelErr } = await supabase
          ...
          .eq('id', bidId);
        if (bidDelErr) {
          console.error('Failed to delete test bid:', bidDelErr.message);
        }
  ...
  if (!dryRun) {
    ...
    if (bidError) {
      console.error('Failed to create bid:', bidError.message);
      process.exit(1);
    }

    bidId = bid.id;
    console.log(`   Created: ${bidId}\n`);
  } else {
  ...
  console.log('Summary');
  console.log('-------');
  console.log(`Bid workspace: ${bidId ?? '(dry run)'}`);
