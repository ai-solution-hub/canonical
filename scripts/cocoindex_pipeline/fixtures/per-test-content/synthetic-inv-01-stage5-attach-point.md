...
Owned by exactly ONE spec: stage-5-attach-point.integration.test.ts
...
Inv-1 observes that per-item entity_mentions are committed under the run's
op_id and that the flow-scope post-pass counter is present. It needs a
document that reliably produces at least one anchored mention; it does not
need a near-match, so this document carries exactly one certification-shaped
token.
-->

# Supplier profile — Calderwood Facilities Group Ltd (Synthetic)
