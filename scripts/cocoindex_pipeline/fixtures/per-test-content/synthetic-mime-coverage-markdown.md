<!--
FIXTURE — per-test CONTENT, staging_mode: per-test (DR-133, amended S543). Owned by exactly ONE spec: sidecar-mime-coverage.integration.test.ts. Do not stage from a second spec: identity in source_documents is content-hash FIRST, so two specs staging identical bytes share one row and the later stagings memo-SKIP. This document exists because Inv-7 asserts each MIME extracts to NON-EMPTY content_chunks, and a blank extraction form is a weak subject for that claim — a form can chunk while carrying no content anyone would want extracted.

MIME under test: markdown. Distinct bytes from every other fixture in this tree
and from the walked baseline; its certification token IIP 60001 is its alone.
-->

# Supplier profile — Ferrybridge Analytics Ltd (Synthetic)
