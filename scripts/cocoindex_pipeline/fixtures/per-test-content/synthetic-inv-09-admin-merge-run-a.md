...
Run A of the Inv-9 pair. Its sibling (synthetic-inv-09-admin-merge-run-b.md) carries
the COMPACT surface form of the same standard, so run B gives Stage-5 a
genuine near-match to resolve — which is what makes 'Stage-5 never reverts
an admin-merged row' a real assertion rather than a walk over an inert
second copy. The two files must never be edited to share a surface form or
to become byte-identical.
-->

# Certification profile — Elmsworth Care Solutions Ltd, assurance record A (spaced form) (Synthetic)
...
The Elmsworth Care Solutions Ltd management system is certified to IEC 62443 by an
...
the current IEC 62443 certificate is issued to buyers at contract award and reissued on
renewal.
