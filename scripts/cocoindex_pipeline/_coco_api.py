...
cocoindex 1.0.18 (verified unchanged since 1.0.7) exposes the symbols this pipeline depends on OUTSIDE its
...
Pinned version: ``cocoindex==1.0.18`` (see ``requirements.txt`` and the
version-pin assertion in ``scripts/tests/test_coco_api_facade.py``, which gates
upgrades). The twelve re-exported symbols and their real sources (verified at 1.0.7 and re-verified at 1.0.18) are
