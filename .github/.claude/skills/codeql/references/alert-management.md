...
Guide for understanding, triaging, dismissing, and resolving code scanning alerts
generated by CodeQL.
...
When a security severity is present, it takes precedence over the standard severity for
display and sorting.
...
These labels are assigned automatically based on file paths. They cannot be manually
overridden.
...
By default, the check fails if alerts have severity of `error`, `critical`, or `high`.
Override this threshold via repository Settings → Rules → Rulesets → Code scanning.
...
GitHub Copilot Autofix automatically generates fix suggestions for CodeQL alerts in pull
requests.
...
- The fix may include changes to files not in the original PR diff (e.g., adding a
...
For false positives from unsupported sanitization libraries, consider contributing to the
CodeQL repository to improve analysis accuracy.
