...
Detailed reference for how CodeQL handles compiled language analysis, including build
modes, autobuild behavior, runner requirements, and hardware specifications.
...
- May be less accurate if code depends heavily on custom macros/defines not in existing
  headers
...
- Ensure external dependencies (headers) are available in system include directories or
  workspace
...
**Supported build systems:** MSBuild, Autoconf, Make, CMake, qmake, Meson, Waf, SCons,
Linux Kbuild, build scripts

### Runner Requirements (C/C++)

- **Ubuntu:** `gcc` compiler; may need `clang` or `msvc`. Build tools: `msbuild`, `make`,
  `cmake`, `bazel`. Utilities: `python`, `perl`, `lex`, `yacc`.
- **Auto-install dependencies:** Set `CODEQL_EXTRACTOR_CPP_AUTOINSTALL_DEPENDENCIES=true`
  (enabled by default on GitHub-hosted; disabled on self-hosted). Requires Ubuntu with
...
- Restores dependencies using heuristics from: `*.csproj`, `*.sln`, `nuget.config`,
  `packages.config`, `global.json`, `project.assets.json`
...
> `/p:EmitCompilerGeneratedFiles=true` may cause issues with legacy projects or `.sqlproj`
> files.
...
1. Invoke `make`, `ninja`, `./build`, or `./build.sh` until one succeeds and
   `go list ./...` works
2. If none succeed, look for `go.mod` (`go get`), `Gopkg.toml` (`dep ensure -v`), or
   `glide.yaml` (`glide install`)
...
> If Kotlin code is added to a repo using `none` mode, disable and re-enable default setup
> to switch to `autobuild`.
...
- Multiple JDK versions — CodeQL uses highest found; lower-version files may be partially
  analyzed
- Colliding class names cause missing method call targets
...
**Runner requirement:** macOS runners only. Not supported on Actions Runner Controller
(ARC) — Linux only.

> macOS runners are more expensive; consider scanning only the build step to optimize
> cost.
...
- Consider `none` build mode where accuracy is acceptable — significantly faster than
  `autobuild`
