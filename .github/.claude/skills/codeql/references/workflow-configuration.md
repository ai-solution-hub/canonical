...
Detailed reference for configuring CodeQL analysis via GitHub Actions workflows. This
supplements the procedural guidance in SKILL.md.
...
- For private fork PRs, enable "Run workflows from fork pull requests" in repository
  settings
...
> **Important:** `paths-ignore` and `paths` control whether the workflow runs. When the
> workflow does run, it analyzes ALL changed files in the PR (including those matched by
> `paths-ignore`), unless files are excluded via the CodeQL configuration file's
> `paths-ignore`.
...
- `security-extended` — additional security queries with slightly higher false-positive
  rate
